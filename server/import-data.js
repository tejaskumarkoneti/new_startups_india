...
function importData(customInput = null) {
  initDatabase();
  ...
  if (Buffer.isBuffer(customInput)) {
    // Input is an uploaded file buffer from Multer
    sourceDescription = 'Uploaded File Buffer';
    workbook = xlsx.read(customInput, { type: 'buffer' });
  } else {
    // Input is a file path or default search
  ...
  // Find sheet containing company data
  ...
  // Query existing CINs to accurately report new vs updated counts
  const existingCinRows = db.prepare('SELECT cin FROM companies').all();
  ...
  const insertStmt = db.prepare(`
    ...
    ON CONFLICT(cin) DO UPDATE SET
      ...
      END,
      guessed_domain = CASE 
        WHEN excluded.guessed_domain != '' THEN excluded.guessed_domain 
        ELSE companies.guessed_domain 
      END,
      status = CASE 
      ...
      END,
      notes_evidence = CASE 
        WHEN excluded.notes_evidence != '' THEN excluded.notes_evidence 
        ELSE companies.notes_evidence 
      END,
...
// Allow passing custom file via CLI argument: node import-data.js my_new_month.xlsx
